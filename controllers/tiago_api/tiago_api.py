...
def main():
    ...
    if wheel_left: wheel_left.setPosition(float("inf"))
    if wheel_right: wheel_right.setPosition(float("inf"))
    print("[tiago_api_1] ✓ Wheels initialized")

    # Head joints
    head_1 = robot.getDevice("head_1_joint")
    head_2 = robot.getDevice("head_2_joint")
    if head_1: head_1.setPosition(0)
    if head_2: head_2.setPosition(0)
    print("[tiago_api_1] ✓ Head joints initialized")

    # Torso lift
    torso_lift = robot.getDevice("torso_lift_joint")
    if torso_lift: torso_lift.setPosition(0)
    ...
    # Safety check for arms
    arm_right_joints = [j for j in arm_right_joints if j is not None]
    ...
    arm_left_joints = [j for j in arm_left_joints if j is not None]
    ...
    while robot.step(timestep) != -1:
                ...
                if cmd_type == "velocity":
                    ...
                    if action == "stop":
                        ...
                        for i, pos in enumerate(arm_home_positions["right"]):
                            if i < len(arm_right_joints): arm_right_joints[i].setPosition(pos)
                        for i, pos in enumerate(arm_home_positions["left"]):
                            if i < len(arm_left_joints): arm_left_joints[i].setPosition(pos)
                        print("[tiago_api_1]   → Moving arms to home position")
                elif cmd_type == "head":
                    h1 = data.get("head_1", 0)
                    h2 = data.get("head_2", 0)
                    if head_1: head_1.setPosition(h1)
                    if head_2: head_2.setPosition(h2)
                    print(f"[tiago_api_1] 👀 HEAD command: pan={h1:.2f}, tilt={h2:.2f}")
                elif cmd_type == "torso":
                    height = data.get("height", 0)
                    if torso_lift: torso_lift.setPosition(height)
                    ...
                    if joint_positions:
                        ...
                        for i, pos in enumerate(joint_positions):
                            if i < len(target_joints): target_joints[i].setPosition(pos)
        ...
        if wheel_left: wheel_left.setVelocity(max(-10, min(10, left_vel)))
        if wheel_right: wheel_right.setVelocity(max(-10, min(10, right_vel)))
