"""Tiago controller #2 that polls Rescue Command Center API and applies all commands."""
...
def fetch_command(api_url, robot_id="2"):
    try:
...
def main():
    ...
    print("[tiago_api_2] Initializing Tiago robot controller...")
    ...
    print("[tiago_api_2] ✓ Wheels initialized")
    ...
    print("[tiago_api_2] ✓ Head joints initialized")

    # Torso lift
    torso_lift = robot.getDevice("torso_lift_joint")
    torso_lift.setPosition(0)
    print("[tiago_api_2] ✓ Torso lift initialized")
    ...
    for joint in arm_right_joints:
        joint.setPosition(0)
    print("[tiago_api_2] ✓ Right arm joints initialized")
    ...
    for joint in arm_left_joints:
        joint.setPosition(0)
    print("[tiago_api_2] ✓ Left arm joints initialized")

    # Astra RGBD camera - must enable for camera panels to display
    try:
        rgb_camera = robot.getDevice("Astra rgb")
        rgb_camera.enable(timestep)
        print("[tiago_api_2] ✓ Astra rgb camera enabled")
    except Exception:
        print("[tiago_api_2] ⚠ Astra rgb camera not found")
    try:
        depth_camera = robot.getDevice("Astra depth")
        depth_camera.enable(timestep)
        print("[tiago_api_2] ✓ Astra depth camera enabled")
    except Exception:
        print("[tiago_api_2] ⚠ Astra depth camera not found")
    ...
    for left_name, right_name in [
        ...
        try:
            ...
            print(f"[tiago_api_2] ✓ Grippers initialized ({left_name}, {right_name})")
            break
        except Exception:
            pass
    if gripper_left is None and gripper_right is None:
        print("[tiago_api_2] ⚠ Grippers not available on this model")
    ...
    print(f"[tiago_api_2] Polling API at {api_url}/tiago/2/command (ensure backend is running: ./run_backend.sh)")
    ...
    while robot.step(timestep) != -1:
        ...
        if poll_counter >= 1:
            poll_counter = 0
            cmd = fetch_command(api_url, "2")
            if cmd and not api_connected:
                api_connected = True
                print("[tiago_api_2] Connected to Rescue Command Center API")
            elif not cmd and not api_connected and warn_counter >= 250:
                warn_counter = 0
                print("[tiago_api_2] ⚠ Backend not reachable at", api_url, "- start backend with ./run_backend.sh")
            ...
            if cmd:
                ...
                if cmd_type == "velocity":
                    ...
                    print(f"[tiago_api_2] 🚀 VELOCITY command: linear_x={linear_x:.2f}, linear_y={linear_y:.2f}, angular={angular:.2f}")
                elif cmd_type == "action":
                    action = data.get("action", "")
                    print(f"[tiago_api_2] ⚡ ACTION command: {action}")
                    if action == "stop":
                        linear_x = linear_y = angular = 0.0
                        print("[tiago_api_2]   → Stopping all movement")
                    ...
                    if joint_positions and len(joint_positions) == 7:
                        ...
                        print(f"[tiago_api_2] 🦾 ARM command: {arm_side} arm → {joint_positions}")
                    ...
                    if arm_side == "right" and gripper_right is not None:
                        ...
                        print(f"[tiago_api_2] ✋ GRIPPER command: {arm_side} → {'OPEN' if action == 'open' else 'CLOSE'}")
                    ...
                    if "x" not in pos and "y" not in pos:
                        ...
                        if last_move_to_target != target_key:
                            ...
                            print("[tiago_api_2]   → Arms to home before move")
                        ...
                        if dist < ARRIVAL_DIST:
                            linear_x = linear_y = angular = 0.0
                            last_move_to_target = None
                            post_move_to_done(api_url, "2")
                            print("[tiago_api_2] 🎯 move_to arrived — stopped (no move, no rotate)")
                        else:
